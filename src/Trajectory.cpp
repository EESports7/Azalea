...
void TrajectoryGameObject::activatedByTrajectory(PlayerObject* pl, int idx){
    auto fields = tm.getFields(this);
...
void TrajectoryGameObject::resetTrajectoryState(){
    auto fields = tm.getFields(this);
...
void TrajectoryGameObject::editTrajectoryState(Activate setting, bool state){
    auto fields = tm.getFields(this);

    if(tm.effectObjects.contains(this))
        tm.effectObjects.emplace(this, fields);
...
void TrajectoryManager::updateTrajectory(bool down){
    ...
    for(uint64_t i = 0; i < lookaheadLength; i++){
        ...
        if(down){
            p1TrajectoryPoints[i] = prevPosP1;
            p2TrajectoryPoints[i] = prevPosP2;
        }
...
bool TrajectoryManager::canBeActivated(PlayerObject* pl, EffectGameObject* obj){
    ...
    auto fields = tm.getFields(static_cast<TrajectoryGameObject*>(obj));
...
void TrajectoryManager::reloadManager(bool move){
    ...
    for(auto [obj, field] : effectObjects)
        if(!field->hasBeenReset) obj->resetTrajectoryState();
}
